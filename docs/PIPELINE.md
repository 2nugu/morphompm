...
[3 Transfer]  P2G → grid → G2P  + VJP  (material-agnostic, calls [2])
     │   single-step forward+VJP ✅ DONE (transfer.py); trajectory ○
[4 Integrate]  operator split: fast mech clock ⟂ slow growth/diffusion clock
     │   trajectory forward + adjoint (compose per-step [2]∘[3] VJPs)   ○
...
- Forward parity: numpy/Taichi vs C++ oracle (det F = g³ exact).
- Determinism: identical runs bit-identical (reproducibility / priority).
- SVD degeneracy (s_i≈s_j, e.g. isotropic growth): clamp + documented error band.
...
3. → [1]+[0] formalize State/Config dataclasses; re-home [2],[3] into package.
4. → [4] trajectory rollout + adjoint (compose per-step VJPs); FD-gated.
5. → [6] inverse design demo (recover growth rate from observed shape).
6. → [2] add Herschel-Bulkley (bioink) constitutive + VJP.
...
(nonsmooth) — FD-gate away from the kink; clamp/​smooth at it. **Not yet coded.**
